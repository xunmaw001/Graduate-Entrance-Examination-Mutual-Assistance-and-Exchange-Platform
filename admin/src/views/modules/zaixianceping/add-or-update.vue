	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.ceshijieguo" label="测试结果" prop="ceshijieguo">
					<file-upload
						tip="点击上传测试结果"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.ceshijieguo?ruleForm.ceshijieguo:''"
						@change="ceshijieguoUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.ceshijieguo" label="测试结果" prop="ceshijieguo">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 25px","margin":"0 20px 0 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#93C7B3","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.ceshijieguo)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.ceshijieguo" label="测试结果" prop="ceshijieguo">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="测试时间" prop="ceshishijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.ceshishijian" 
						type="date"
						:readonly="ro.ceshishijian"
						placeholder="测试时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.ceshishijian" label="测试时间" prop="ceshishijian">
					<el-input v-model="ruleForm.ceshishijian" placeholder="测试时间" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.ceshishijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='ceshijieguo'){
							this.ruleForm.ceshijieguo = obj[o];
							this.ro.ceshijieguo = true;
							continue;
						}
						if(o=='ceshishijian'){
							this.ruleForm.ceshishijian = obj[o];
							this.ro.ceshishijian = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
	...
	if(this.ruleForm.ceshijieguo!=null) {
		this.ruleForm.ceshijieguo = this.ruleForm.ceshijieguo.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaixianceping/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaixiancepingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    ceshijieguoUploadChange(fileUrls) {
	    this.ruleForm.ceshijieguo = fileUrls;
    },
  }
};
</script>
