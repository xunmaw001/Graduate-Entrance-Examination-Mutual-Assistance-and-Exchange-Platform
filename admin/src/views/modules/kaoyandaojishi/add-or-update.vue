	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="倒计时" prop="daojishi">
					<el-input v-model="ruleForm.daojishi" placeholder="倒计时" clearable  :readonly="ro.daojishi"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="倒计时" prop="daojishi">
					<el-input v-model="ruleForm.daojishi" placeholder="倒计时" readonly></el-input>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='daojishi'){
							this.ruleForm.daojishi = obj[o];
							this.ro.daojishi = true;
							continue;
						}
						if(o=='neirong'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kaoyandaojishi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kaoyandaojishiCrossAddOrUpdateFlag = false;
						 this.parent.search();
