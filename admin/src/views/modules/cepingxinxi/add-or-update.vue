	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.cepingtimu" label="测评题目" prop="cepingtimu">
					<file-upload
						tip="点击上传测评题目"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.cepingtimu?ruleForm.cepingtimu:''"
						@change="cepingtimuUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.cepingtimu" label="测评题目" prop="cepingtimu">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 25px","margin":"0 20px 0 0","outline":"none","color":"#fff","borderRadius":"30px","background":"#93C7B3","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.cepingtimu)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.cepingtimu" label="测评题目" prop="cepingtimu">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="测评简介" prop="cepingjianjie">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.cepingjianjie" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.cepingjianjie" label="测评简介" prop="cepingjianjie">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.cepingjianjie"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='cepingtimu'){
							this.ruleForm.cepingtimu = obj[o];
							this.ro.cepingtimu = true;
							continue;
						}
						if(o=='cepingjianjie'){
							this.ruleForm.cepingjianjie = obj[o];
							this.ro.cepingjianjie = true;
							continue;
						}
						if(o=='faburiqi'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.cepingjianjie = this.ruleForm.cepingjianjie.replace(reg,'../../../phpl3663/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.cepingtimu!=null) {
		this.ruleForm.cepingtimu = this.ruleForm.cepingtimu.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cepingxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cepingxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    cepingtimuUploadChange(fileUrls) {
	    this.ruleForm.cepingtimu = fileUrls;
    },
  }
};
</script>
