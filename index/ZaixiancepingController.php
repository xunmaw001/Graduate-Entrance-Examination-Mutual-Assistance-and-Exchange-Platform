...
session_start();
class ZaixiancepingController extends CommonController {
    ...
    public function page(){
                ...
                if($base['tablename'] == 'yonghu'){
                    $orsign = false;
                        ...
                        $where .= " and `yonghuzhanghao` = '".$_SESSION[$md5]."'";
                                        ...
                                        $sql = "select * from `zaixianceping` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
            ...
            $lists = "select * from `zaixianceping` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `zaixianceping` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
        $sql = "UPDATE zaixianceping SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from zaixianceping WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `zaixianceping` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $where = " where 1 ";
            ...
            $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                                                                                ...
                                                                                $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM zaixianceping ".$where." group by ".$xColumnName." LIMIT 10";
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM zaixianceping ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM zaixianceping ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM zaixianceping ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function group($columnName){
        ...
        $where = " where 1 ";
            ...
            $where .= " and `yonghuzhanghao` = '".$tokens['username']."' ";
                                                                                ...
                                                                                $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM zaixianceping ".$where." GROUP BY ".$columnName." ORDER BY total desc LIMIT 10";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
