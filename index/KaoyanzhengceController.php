...
session_start();
class KaoyanzhengceController extends CommonController {
    ...
    public function page(){
			...
			if(in_array($k, $this->columData)){
                if ($val != ''){
                    $where.= " and ".$k." like '".$val."'";
                                        ...
                                        $sql = "select * from `kaoyanzhengce` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
            ...
            $lists = "select * from `kaoyanzhengce` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `kaoyanzhengce` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
        $sql = "UPDATE kaoyanzhengce SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from kaoyanzhengce WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                ...
                $this->andone('kaoyanzhengce',$id,$column='clicknum',$num=1);
                        $clicktime = "update kaoyanzhengce set clicktime = ".date('Y-m-d H:i:s',time())." where ".$where;
        table_sql($clicktime);
                $sql = "select * from `kaoyanzhengce` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
                $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM kaoyanzhengce ".$where." group by ".$xColumnName." LIMIT 10";
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM kaoyanzhengce ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM kaoyanzhengce ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM kaoyanzhengce ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
		...
		$limt = isset($_REQUEST['limit'])?$_REQUEST['limit']:"5";
				$sort = 'clicknum';
        		$lists = "select * from `kaoyanzhengce` order by ".$sort." desc limit 0,".$limt;
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
			while ($datas = $result->fetch_assoc()){
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
                $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM kaoyanzhengce ".$where." GROUP BY ".$columnName." ORDER BY total desc LIMIT 10";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
